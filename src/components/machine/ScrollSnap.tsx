...
/* ═══════════════════════════════════════════════════════════
   ...
   スクロールが必ずどれかのユニットの読み位置で止まるようにする。
...
export function ScrollSnap({ regionId }: { regionId: string }) {
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (inside !== armed) {
        armed = inside
        root.style.scrollSnapType = inside ? 'y mandatory' : ''
      }
    }
